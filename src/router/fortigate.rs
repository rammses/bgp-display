...
impl FortiGateBackend {
    ...
    pub async fn write_config(&self, commands: &[String]) -> Result<()> {
        let out = self
            .run_cli_pipeline(&commands.iter().map(|s| s.as_str()).collect::<Vec<_>>())
            .await
            .context("write_config: FortiGate CLI pipeline failed")?;

        if out.contains("Command fail") || out.contains("Unknown action") {
