...
pub fn draw(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let (content_area, filter_area) = if app.config_filter_mode != FilterMode::Off {
...
fn draw_config_list(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let use_filter = app.config_filter_mode != FilterMode::Off && !app.config_indices.is_empty();
    ...
    let items: Vec<ListItem> = if app.config_lines.is_empty() {
        ...
        app.config_indices
            .iter()
            .filter_map(|&i| app.config_lines.get(i))
    ...
    let title = if use_filter {
        format!(
            " BGP Config: {router_name} ({}/{}) ",
            app.config_indices.len(),
            app.config_lines.len()
        )
    } else {
        format!(" BGP Config: {router_name} ")
    };
